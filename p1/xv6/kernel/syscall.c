...
// Counter for total system calls
unsigned int counterTotal = 0;
// Counter for success system calls
...
syscall(void)
{
  ...
  if(num > 0 && num < NELEM(syscalls) && syscalls[num] != NULL) {
	// increment the counter for total syscall before syscall issued
	counterTotal++;
	int ret = syscalls[num]();
	if (ret >= 0) {
      // increment the counter for success syscall if not -1 returned
  	  counterSuccess++;
    }
    proc->tf->eax = ret;
  } else {
