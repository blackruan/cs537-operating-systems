...
uint csget(struct inode *ip, uint bn);
uint cscompute(uchar *data);
int csupdate(struct inode *ip, uint bn, uint checksum);
  ...
  if(ip->type == T_CHECKED) {
    ...
    for(i = 0; i < (NDIRECT + 1); i++) {
      if(i == NDIRECT) {
        ...
        int j;
        for(j = 0; j < NINDIRECT; j++)
          checksum ^= (uchar) ((a[j] & 0xff000000) >> 24);
        brelse(bp);
      } else {
        checksum ^= (uchar) ((ip->addrs[i] & 0xff000000) >> 24);
      }
    }
...
readi(struct inode *ip, char *dst, uint off, uint n)
{
  ...
  for(tot=0; tot<n; tot+=m, off+=m, dst+=m){
    ...
    if(ip->type == T_CHECKED) {
      if(csget(ip, off/BSIZE) != cscompute(bp->data)) {
        brelse(bp);
        return -1;
      }
    }
...
writei(struct inode *ip, char *src, uint off, uint n)
{
  ...
  for(tot=0; tot<n; tot+=m, off+=m, src+=m){
    ...
    bwrite(bp);
    if(ip->type == T_CHECKED)
      csupdate(ip, off/BSIZE, cscompute(bp->data));
    brelse(bp);
  }
...
// get the checksum field of the data block
uint
csget(struct inode *ip, uint bn)
{
  struct buf *bp;
  uint *a;
  uint checksum;
  ...
  panic("get checksum failed");
}

// compute checksum for data block
uint
cscompute(uchar *data)
{
  uint rv;
  uchar checksum = 0;
  int i;
...
// update the checksum field for one data block for its inode
int
csupdate(struct inode *ip, uint bn, uint checksum)
{
  struct buf *bp;
  uint *a;
  if(bn < NDIRECT) {
    ip->addrs[bn] &= 0x00ffffff;
    ip->addrs[bn] |= checksum;
    return 0;
  }
  bn -= NDIRECT;
  if(bn < NINDIRECT) {
